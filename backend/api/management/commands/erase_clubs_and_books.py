...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"✓ Deleted {book_interest_count} book interest records"))

            # Delete all event attendances
            EventAttendance.objects.all().delete()
            self.stdout.write(self.style.SUCCESS(f"✓ Deleted {event_attendance_count} event attendance records"))
            ...
            self.stdout.write(self.style.SUCCESS(f"✓ Deleted {book_count} books"))

            # Delete all club memberships
            ClubMembership.objects.all().delete()
            self.stdout.write(self.style.SUCCESS(f"✓ Deleted {membership_count} club memberships"))

            # Delete all clubs
            Club.objects.all().delete()
            self.stdout.write(self.style.SUCCESS(f"✓ Deleted {club_count} clubs"))
