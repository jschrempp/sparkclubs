...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    def is_club_admin(self, club: "Club") -> bool:
...
class Club(models.Model):
    ...
    is_public = models.BooleanField(
        default=True, help_text="Public clubs are visible to all users. Private clubs are only visible to members."
    )
    auto_approve_topics = models.BooleanField(
        default=False,
        help_text="Automatically approve topics when added. When disabled, club admins must manually approve topics.",
    )
    invite_token = models.CharField(
        ...
        blank=True,
        help_text=(
            "Opaque token used in invite links (/join/<token>) instead of "
            "the numeric id, so club ids cannot be guessed or enumerated."
        ),
    )
...
class SystemSettings(models.Model):
    ...
    def get_settings(cls) -> "SystemSettings":
        ...
        settings, _created = cls.objects.get_or_create(setting_id=1)
        return settings
